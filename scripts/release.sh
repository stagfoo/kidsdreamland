...
# and this app repaints a canvas under a moving finger — the one thing a JIT
# build is visibly worse at. It is also ~17MB against ~80MB. Signed with the
# same committed debug.keystore, so it still installs as an update over
